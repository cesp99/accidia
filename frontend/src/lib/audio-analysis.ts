 ...
 *
 * Used by the progressive-load path in App.tsx: we start playback
 * immediately with this minimal analysis (so the user sees no delay)
 * and swap in the real analysis once the heavy beat-detection finishes
 * in the background.
 *
 ...
 * need edges which require real analysis; we just keep edges empty).
 */
