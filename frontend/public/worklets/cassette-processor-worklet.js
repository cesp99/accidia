// Cassette lo-fi processor: bitcrush + sample-rate reduction + low-pass + subtle noise
// Adapted from YouTube Beatmaker Cues (owae.ga) — Web Audio AudioWorklet.
class CassetteProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.port.onmessage = (event) => {
      ...
      if (Object.prototype.hasOwnProperty.call(data, "bitDepth")) {
        this.bitDepth = data.bitDepth;
      }
      if (Object.prototype.hasOwnProperty.call(data, "targetSampleRate")) {
        this.targetSampleRate = data.targetSampleRate;
        this.step = Math.max(1, Math.floor(sampleRate / this.targetSampleRate));
      }
      if (Object.prototype.hasOwnProperty.call(data, "cutoff")) {
        this.cutoff = data.cutoff;
      }
      if (Object.prototype.hasOwnProperty.call(data, "noiseAmp")) {
  ...
  process(inputs, outputs) {
    ...
    for (let channel = 0; channel < input.length; channel++) {
      ...
      if (this.prevFiltered[channel] === undefined) this.prevFiltered[channel] = 0;
      if (this.lastSamples[channel] === undefined) this.lastSamples[channel] = 0;
